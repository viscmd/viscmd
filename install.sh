...
/bin/cp viscmd-completion-*.sh /bin/
chmod +x /bin/viscmd-completion-*.sh
...
zshrc=$(eval echo ~$SUDO_USER/.zshrc)
if [ -e $zshrc ]; then
	if ! grep -q viscmd-completion-zsh.sh $zshrc; then
		echo >>$zshrc
		echo '# visual command' >>$zshrc
		echo 'source /bin/viscmd-completion-zsh.sh' >>$zshrc
	fi
